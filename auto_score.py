...
port_color = {1: "Red", 2: "Blue", 3: "Yellow", 4: "Green"}
...
def get_winner(g):
    ...
    stocks_winner = (
        port1
        if p1.leader.post.stocks > p2.leader.post.stocks
        else port2
        if p1.leader.post.stocks < p2.leader.post.stocks
        else None
    )
    percent_winner = (
        port1
        if p1.leader.post.damage < p2.leader.post.damage
        else port2
        if p1.leader.post.damage > p2.leader.post.damage
        else None
    )
    winner = stocks_winner if stocks_winner is not None else percent_winner

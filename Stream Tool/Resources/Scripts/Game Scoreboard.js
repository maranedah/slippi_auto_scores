	...
	//now things that will happen constantly
		...
		if (bestOfPrev != bestOf) {
			if (bestOf == "Bo5") {
				...
				fadeOut("#bestOf", () => {
					document.getElementById('bestOf').textContent = "Best of 5";
					document.getElementById('bestOf').size = 10;
					fadeIn("#bestOf");
				});
			} else {
				gsap.to('#win3P1',
...
function updateColor(colorID, textID, pColor) {
	...
	//gsap.to(colorEL, {backgroundColor: "#ffffff", duration: fadeInTime});
...
function updatePlayerName(wrapperID, nameID, teamID, pName, pTeam) {
	...
	nameEL.style.fontSize = '20px'; //set original text size
...
async function updateChar(pCharacter, pSkin, charID, sagaID) {
	...
	charEL.setAttribute('src', 'Resources/Characters/Portraits2/' + pCharacter + '/' + pSkin + '.png');
